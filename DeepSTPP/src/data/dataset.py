...
class Pre(torch.utils.data.Dataset):
    ...
    """
    Take a batch of sequences and create fixed length dataset without applying sliding window.
    
    seqs: list of [seqlen, 3] np.array
    
    self.st_X: torch.tensor, [N, lookback] -> Training Examples
    self.st_Y: torch.tensor, [N, seq_len - lookback] -> Training Labels
    """
    def __init__(self, seqs, lookback=2, normalized=False, min=None, max=None):
        self.seqs_cum = seqs
        self.seqs = deepcopy(self.seqs_cum)
        for i, seq in enumerate(self.seqs):
            self.seqs[i][:, 0] = np.diff(seq[:, 0], axis=0, prepend=0)
        ...
        for seq_i, (seq, seq_cum) in enumerate(zip(self.seqs, self.seqs_cum)):     
            for i in range(lookback, len(seq)):
                self.st_X_cum.append(seq_cum[:lookback])
                self.st_Y_cum.append(seq_cum[lookback:])
                
                self.st_X.append(seq[:lookback])
                self.st_Y.append(seq[lookback:])
